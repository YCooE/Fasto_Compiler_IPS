...
let rec compileExp  (e      : TypedExp)
  ...
  | Constant (BoolVal b, pos) ->
      if b then [ Mips.LI (place, makeConst (int 1)) ]
      else [ Mips.LI (place, makeConst (int 0)) ]
  ...
  *)
  | Replicate (n_exp, elem_exp, elem_type, pos) ->
      ...
      let i_reg    = newName "ind_var"  (* loop counter *)
      let tmp_reg  = newName "tmp_reg"  (* several purposes *)
      let loop_beg = newName "loop_beg" (* loop begin *)
      let loop_end = newName "loop_end" (* loop end *)

      let safe_lab = newName "safe_lab"
      let checksize = [ Mips.ADDI (size_reg, size_reg, "-1")
                      ; Mips.BGEZ (size_reg, safe_lab)
                      ; Mips.LI ("5", makeConst(elemSizeToInt elem_size))
                      ; Mips.J "_IllegalArrSizeError_"
                      ; Mips.LABEL (safe_lab)
                      ; Mips.ADDI (size_reg, size_reg, "1")
                      ]
  ...
  *)
  | Filter (farg, arr_exp, elem_type, pos) ->
      ...
      let elem_reg = newName "elem_reg" (* address of single element *)
      let res_reg  = newName "res_reg"
      let c_reg = newName "counter_reg" (* counter for predicate*)
      let arr_code = compileExp arr_exp vtable arr_reg
      ...
      let loop_header = [ Mips.LABEL (loop_beg)
                        ; Mips.SUB (tmp_reg, i_reg, size_reg)
                        ; Mips.BGEZ (tmp_reg, loop_end) ]
      (* Result of predicate function for a given element *)
      let loop_filter0 =
          match getElemSize elem_type with
            | One  -> Mips.LB(res_reg, elem_reg, "0")
      ...
      let pred_false = newName"pred_false"
      (* If Predicate is true move element to result *)
      let loop_filter1 = [ Mips.BEQ   (tmp_reg, "0", pred_false)
                         ; Mips.ADDI  (c_reg, c_reg, "1")
                         ]
      (* Store result if predicate is true*)
      let loop_filter2 =
          match getElemSize elem_type with
          | One  -> [ Mips.SB (res_reg, addr_reg, "0") ]
          | Four -> [ Mips.SW (res_reg, addr_reg, "0") ]
      ...
      let loop_footer =
              ...
              ; Mips.LABEL (pred_false)
              ; Mips.ADDI (i_reg, i_reg, "1")
              ; Mips.J loop_beg
              ; Mips.LABEL loop_end
              ; Mips.SW(c_reg, place, "0")
              ]
      arr_code
       @ get_size
       @ dynalloc (size_reg, place, elem_type)
