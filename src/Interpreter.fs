  ...
  | Filter (farg, arrexp, _, pos) ->
        ...
        match arr with
          | ArrayVal(lst, tp1) ->
               ...
               if farg_ret_type = Bool then ArrayVal (mlst, tp1)
               else raise (MyError ("Return type of function must be of type bool: "+ppVal 0 arr, pos))
  ...
  *)
  | Scan (farg, ne, arrexp, _, pos) ->
      ...
      match arr with
        | ArrayVal (lst,tp1) ->
            let mlst =
                      match List.scan(fun acc x -> evalFunArg (farg, vtab, ftab, pos, [acc;x])) nel lst with
                        | a::b -> b
                        | a -> a
            ArrayVal (mlst, tp1)
